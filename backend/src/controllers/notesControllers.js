...
export async function getAllnotes(_,res){
    try{
        const notes = await Note.find({}).sort({createdAt: -1}); // Fetch all notes sorted by createdAt in descending order
...
export async function createNote(req,res){
    try{
        ...
        res.status(500).json({message: "Internal Server Error"});///display on the website...
    }
...
export async function updateNote(req,res){
    try{
        ...
        res.status(200).json(updatedNote);//response
    }catch(error){
